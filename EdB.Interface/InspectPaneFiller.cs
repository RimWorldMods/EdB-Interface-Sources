	...
	internal class InspectPaneFiller
	{
		...
		public static void DrawHealth(WidgetRow row, Thing t)
		{
			...
			if (pawn == null)
			{
				...
				else if ((double)((float)t.HitPoints) > (double)((float)t.MaxHitPoints) * 0.5)
				{
		...
		public static void DrawInspectStringFor(ISelectable t, ref float y)
		{
			...
			Rect rect = new Rect(0f, y, RimWorld.MainTabWindow_Inspect.PaneInnerSize.x, 100f);
		...
		private static void DrawMood(WidgetRow row, Pawn pawn)
		{
			...
			row.DoGap(6f);
			row.DoBar(93f, 16f, pawn.needs.mood.CurLevel, pawn.needs.mood.MoodString.CapitalizeFirst(), InspectPaneFiller.MoodTex, InspectPaneFiller.BarBGTex);
		}
